...
export class SceneManager {
    ...
    initDragControls(model) {
        ...
        this.dragControls.onUpdateJoint = (joint, angle) => {
            ...
            // If external handler exists (for robot connection), let it handle
            // When connected: sends command to robot, doesn't update visualization
            // Visualization is updated by robot state feedback (digital twin principle)
            if (this.onJointDragUpdate) {
                const handled = this.onJointDragUpdate(joint, angle, model);
                if (handled) {
                    // Robot is connected - don't update visualization here
                    // It will be updated by robot state feedback
                    return;
                }
            }
