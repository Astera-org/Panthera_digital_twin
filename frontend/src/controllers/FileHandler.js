...
export class FileHandler {
    ...
    async loadFromServer(_baseUrl, fileList) {
        try {
            ...
            // Find the URDF file (prefer xrb.urdf to match backend config)
            let urdfPath = null;
            const urdfFiles = Object.keys(fileList).filter(p => p.endsWith('.urdf'));

            // Prefer xrb.urdf if available (matches backend robot config)
            urdfPath = urdfFiles.find(p => p.includes('xrb.urdf')) || urdfFiles[0];

            if (!urdfPath) {
                console.error('No URDF file found in arm_description');
                return;
            }
